...
const RoomEntry = ({ token }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-100 to-gray-200 text-gray-900 flex flex-col items-center p-6">
      <nav className="w-full flex justify-between items-center px-16 py-2 shadow-md shadow-blue-100 rounded-lg">
        <div>
      ...
      <main className="w-full max-w-3xl text-center mt-10">
        <h2 className="text-[2.3rem] font-medium">
          Code{" "}
          <sup className="text-blue-600 text-[2.5rem] shadow-md rounded-xl px-3 py-2">
            Together
          </sup>{" "}
          Anytime, Anywhere
        </h2>
        <p className="text-[1.3rem] font-normal text-gray-500 leading-relaxed">
        ...
        <h3 className="text-2xl font-medium mt-20">Available Rooms</h3>
      ...
      <div className="w-full overflow-hidden whitespace-nowrap shadow-sm py-3 mt-20 border-t border-b border-gray-300  rounded-lg">
        <div className="flex space-x-8 w-max animate-marquee">
          {features.concat(features).map((feature, index) => (
            <div
              key={index}
              className="p-4  bg-gray-100 rounded-lg flex items-center space-x-4 min-w-[250px] shadow-md transition-transform transform hover:scale-105"
            >
              <img
                loading="lazy"
                src={feature.img}
